...
# - The requred data set is downloaded and unzipped in current working directory.
...
# define helper functions to trim stings
# returns string w/o leading white space
trim.leading <- function (x)  sub("^\\s+", "", x)

# returns string w/o trailing white space
...
load_data <- function(datasetname) {
  fSubject = paste("UCI HAR Dataset/", datasetname, "/subject_", datasetname, ".txt", sep="")
  fDataX = paste("UCI HAR Dataset/", datasetname, "/X_", datasetname, ".txt", sep="")
  fDataY = paste("UCI HAR Dataset/", datasetname, "/y_", datasetname, ".txt", sep="")
  fActivity = "UCI HAR Dataset/activity_labels.txt"
...
t_test <- next_block(con, nrows=nlines)       # read the full file in one call
...
# activity_id, acrivity, subject, measure1, measure2, measure3, ...
test <- data.frame(act_test,s_test, t_test)


# the following steps are the same as on the test data set just for the train data set.
...
nlines <- determine_nlines("UCI HAR Dataset/train/subject_train.txt")
...
t_train <- next_block(con, nrows=nlines)
close(con)
...
total <- rbind(load_data("train"),load_data("test"))
